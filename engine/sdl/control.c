...
#if WIN || LINUX || DARWIN
// TODO: Might possibly add include guard if decided to only enable devconsole feature at compile time
...
#if WIN || LINUX || DARWIN
        // put devconsole's key handler below above
        // in order for users to be able to switch full / windowed screen
        if (devconsole_control_update(keystate, ev))
          // if consumed, then break now
          break;
#endif
...
#ifdef SDL2
#if WIN || LINUX || DARWIN
      // block of code for supporing text input in devconsole
      // it's here to put more priority in executing normal key control
      // it's ok to check this last when devconsole is shown as we don't need real-time game interaction
      case SDL_TEXTINPUT:
        if (devconsole_control_update(keystate, ev))
          // if consumed, then break now
          break;
